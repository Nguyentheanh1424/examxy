...
- Treat this docs portal as the entrypoint before reading module instructions or changing repo-tracked files.
...
- Use `planning/backend-development-tasks.md` for backend backlog and implementation-ready task breakdowns that are not shipped behavior.
- Use GitNexus alongside these docs for code execution work: repo context first, then `query`/`context` for exploration, `impact` before editing symbols, and `detect_changes` before commit.
...
- `docs/planning/backend-development-tasks.md`
